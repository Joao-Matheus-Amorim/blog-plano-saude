...
function renderMarkdown(content = '') {
  return content
    ...
    .map((block) => {
      ...
      if (!text) return '';
      if (text.startsWith('### ')) {
        return `<h3 style="font-size: clamp(21px, 3vw, 26px); color: #8B7E74; font-weight: 500; margin: 36px 0 18px;">${escapeHtml(text.replace('### ', ''))}</h3>`;
      }
      if (text.startsWith('## ')) {
        return `<h2 style="font-size: clamp(24px, 3.5vw, 32px); color: #8B7E74; font-weight: 400; margin: 48px 0 24px; font-family: 'Playfair Display', serif;">${escapeHtml(text.replace('## ', ''))}</h2>`;
      }
      if (text.startsWith('- ')) {
        const items = text.split('\n').map(item => `<li style="margin-bottom: 10px;">${escapeHtml(item.replace(/^- /, ''))}</li>`).join('');
        return `<ul style="margin: 0 0 24px 22px; padding: 0;">${items}</ul>`;
      }
      return `<p style="margin-bottom: 24px;">${escapeHtml(text).replaceAll('\n', '<br/>')}</p>`;
    })
    .join('');
}
...
export default function PaginaBlogPost() {
  ...
  const carregarPost = async () => {
    try {
      ...
      try {
        ...
        allPosts = seedBlogPosts.map(normalizePost).filter(item => item.ativo);
  ...
  return (
    <>
      ...
      <div className="blog-post-page" style={{ paddingTop: 'clamp(100px, 12vh, 140px)', background: 'linear-gradient(180deg, #FAF8F5 0%, #FFFFFF 100%)', minHeight: '100vh', opacity: 1, visibility: 'visible' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 clamp(20px, 5vw, 40px) clamp(60px, 8vh, 80px)' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 clamp(20px, 5vw, 40px) clamp(60px, 8vh, 80px)', textAlign: 'center' }}>
