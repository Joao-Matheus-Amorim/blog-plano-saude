...
//   <OperatorCards />        ← seção de operadoras
...
// Salva lead no Neon via /api/leads e dispara fbq com eventID para deduplicação CAPI.
// Nunca bloqueia o fluxo do usuário — erros são silenciosos.
async function saveLead(payload) {
  ...
  try {
    await fetch("/api/leads", {
      ...
      body: JSON.stringify({ ...payload, origem: utm_source, event_id: eventId }),
    });
  } catch (_) {}

  // Pixel com mesmo eventId → Meta deduplica com a CAPI server-side
  if (window.fbq) {
...
export function ScarcityBar() {
  ...
  return (
    <div style={{
      ...
      gap: 12, fontSize: "clamp(12px, 1.5vw, 14px)", fontWeight: 500,
    }}>
...
export function LeadMagnetSection() {
  ...
  const handleSend = async () => {
    ...
    await saveLead({ nome: name, telefone: whatsapp, source: "lead_magnet" });
      ...
      <div style={cardStyle}>
        ...
        <div>
          <div style={{
            ...
            <h3 style={{ fontFamily: font, color: C.primary, fontSize: 22, marginBottom: 6, fontWeight: 600 }}>
...
const testimonials = [
  {
    name: "Carlos Mendonça", role: "Sócio — Mendonça Advocacia",
    ...
    stars: 5, initials: "CM", color: "#A8877A",
  },
  {
    name: "Fernanda Costa", role: "Nutricionista • MEI",
    ...
    stars: 5, initials: "FC", color: "#8B7E74",
  },
  {
    name: "Roberto e Ana Lima", role: "Família • 4 membros",
    ...
    stars: 5, initials: "RL", color: "#C5BCB5",
  },
];

export function TestimonialsSection() {
  return (
    <section style={{ padding: "clamp(80px, 10vw, 120px) clamp(20px, 5vw, 60px)", background: "#fff" }}>
      ...
      <div style={{
        ...
        {testimonials.map((t, i) => (
          <div key={i} style={{
            background: "#FAFAF9", border: "1px solid rgba(197,188,181,0.25)",
            borderRadius: 20, padding: "clamp(24px, 3.5vw, 36px)",
            ...
            <p style={{ fontSize: 15, color: C.text, lineHeight: 1.8, fontStyle: "italic", flex: 1 }}>
              "{t.text}"
            </p>
...
export function SavingsCalculator() {
  ...
  const calculate = () => {
    ...
    const optimizedCost = marketCost * 0.73;
  ...
  const handleGetFull = async () => {
    ...
    await saveLead({
      nome: captureName,
      telefone: captureWA,
      mensagem: `Tipo: ${planType} • Vidas: ${lives}`,
      source: "calculator",
    });
      ...
      <div style={{
        ...
        {result && (
          <div>
            ...
            {showCapture && !sent && (
              <div style={{ background: "#F0FFF4", borderRadius: 14, padding: 20, border: "1px solid rgba(37,211,102,0.2)" }}>
            ...
            {sent && (
              <div style={{ textAlign: "center", padding: "16px", background: "#F0FFF4", borderRadius: 14, border: "1px solid rgba(37,211,102,0.2)" }}>
                <p style={{ fontWeight: 700, color: "#276749", margin: 0 }}>✅ Enviado! Verifique o WhatsApp.</p>
              </div>
            )}
          </div>
        )}
      ...
      <div style={{
        ...
        {operators.map((op, i) => (
          ...
          >
            <div
              style={{
                padding: op.highlight ? "clamp(28px, 4vw, 44px) clamp(24px, 3vw, 36px)" : "clamp(20px, 3vw, 32px)",
                background: "rgba(255,255,255,0.95)", backdropFilter: "blur(20px)",
                border: `${op.highlight ? "2" : "1"}px solid ${op.color}${op.highlight ? "30" : "14"}`,
                borderRadius: 20, display: "flex", flexDirection: "column",
                alignItems: "center", justifyContent: "center",
                minHeight: op.highlight ? 180 : 140,
                boxShadow: op.highlight ? `0 12px 48px ${op.color}20` : "0 8px 32px rgba(139,126,116,0.08)",
                cursor: "pointer", transition: "transform 0.2s, box-shadow 0.2s", position: "relative",
              }}
              onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = `0 20px 60px ${op.color}30`; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = op.highlight ? `0 12px 48px ${op.color}20` : "0 8px 32px rgba(139,126,116,0.08)"; }}
            >
              {op.tag && (
                <div style={{
              ...
              <span style={{
                ...
                color: op.color, fontFamily: font,
                textAlign: "center", marginBottom: 12, lineHeight: 1.3,
              }}>
                {op.name}
              </span>
              <span style={{ fontSize: 12, fontWeight: 600, color: op.color, textTransform: "uppercase", letterSpacing: "0.08em", opacity: 0.8 }}>
